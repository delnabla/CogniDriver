// This code belongs to Emotiv with small modifications on how we display the data.
...
{
	var i:int;
	var engine:EmoEngine = EmoEngine.Instance;
	node = new int[18];
	for (i = 0 ; i<18 ;i++)
 	{
  	  node[i] = 0;
	...
	headArea = Rect(10, 10, 225*int_scale, 200*int_scale);
...
function OnGUI()
{
    isEnable = GUI.Toggle(new Rect(5,5,100,50), isEnable, "Show info");
	if (isEnable) DrawGUI();
}
